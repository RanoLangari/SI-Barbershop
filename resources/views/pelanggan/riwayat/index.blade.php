            ...
            <div class="col-md-4 mb-4 reservation-item fade-in">
                <div class="card reservation-card h-100">
                    <div class="card-body">
                        ...
                        <p class="card-text"><strong>Kategori:</strong> {{ $reservasi->kategori->nama }}
                        </p>
                        ...
                        <div class="mt-2">
                            <button class="btn btn-primary btn-sm pay-button" data-id="{{ $reservasi->id }}">
                                <i class="fas fa-money-bill-wave"></i> Bayar Sekarang
                            </button>
                            <button class="btn btn-danger btn-sm cancel-button" data-id="{{ $reservasi->id }}">
                        ...
                        <div class="mt-2">
                            ...
                            <div class="d-flex flex-column align-items-start">
                                ...
                                <button class="btn btn-secondary btn-sm view-proof-button"
                                    data-bukti-id="{{ $reservasi->id }}">
                                    <i class="fas fa-eye"></i> Lihat Bukti
                                </button>

                                <script>
                                document.querySelector('[data-bukti-id="{{ $reservasi->id }}"]').addEventListener(
                                    'click',
                                    function() {
                                        Swal.fire({
                                            title: 'Bukti Refund',
                                            imageUrl: '{{ asset('
                                            uploads / refund / ' . $refundRecord->bukti) }}',
                            ...
                            <div class="d-flex gap-2">
                                <button class="btn btn-info btn-sm refund-button mr-2" data-id="{{ $reservasi->id }}">
                                    <i class="fas fa-undo-alt"></i> Ajukan Refund
                                </button>
                                <button class="btn btn-warning btn-sm reschedule-button-custom"
                                    data-id="{{ $reservasi->id }}" data-barberman="{{ $reservasi->barberman->id }}"
                                    data-tanggal="{{ $reservationDate }}" data-jadwal="{{ $reservasi->jadwal->id }}">
                                ...
                                document.addEventListener('DOMContentLoaded', function() {
                                    document.querySelectorAll('.reschedule-button-custom').forEach(button => {
                                        button.addEventListener('click', function() {
                                            ...
                                            const minDate = tomorrow.toISOString().split('T')[
                                                0];
                                            ...
                                            Swal.fire({
                                                ...
                                                if (result.isConfirmed) {
                                                    ...
                                                    loadTimeSlots(barbermanId, tanggal,
                                                        reservasiId);
                                                }
                                            });
                                        });
                                        ...
                                        fetch(`/get-barberman-schedule/${barbermanId}?tanggal=${date}`)
                                            ...
                                            .then(slots => {
                                                ...
                                                Swal.fire({
                                                    ...
                                                    if (result.isConfirmed) {
                                                        submitReschedule(reservasiId, date, result
                                                            .value);
                                                    }
                                                });
                                            })
                                            ...
                                            .then(data => {
                                                if (data.status === 'success') {
                                                    ...
                                                    Swal.fire({
                                                        ...
                                                        title: 'Gagal!',
                                                        text: data.message ||
                                                            'Terjadi kesalahan saat mengubah jadwal'
                                                    });
                                                }
                                            })
                                            .catch(error => {
                        ...
                        <script>
                        document.querySelector('[data-id="{{ $reservasi->id }}"]').addEventListener('click',
                            function() {
                                Swal.fire({
                                    title: 'Ajukan Refund',
                                    html: `
                                    ...
                                    if (result.isConfirmed) {
                                        ...
                                        formData.append('address_refund', document.getElementById(
                                            'address_refund').value);
                                        formData.append('address_name', document.getElementById(
                                            'address_name').value);
                                        ...
                                        fetch('{{ route('
                                            ...
                                            .then(data => {
                                                if (data.status === 'success') {
                                                    Swal.fire('Berhasil!',
                                                            'Pengajuan refund telah dikirim', 'success')
                                                        .then(() => {
                                                            window.location.reload();
                                                        });
                                                } else {
                        ...
                        @if ($reservasi->status == 'done' || ($reservasi->status == 'confirmed' &&
                        now()->gt($reservationDateTime)))
                        <div class="mt-2">
                            @php
                            ...
                            @else
                            <button class="btn btn-success btn-sm review-button" data-id="{{ $reservasi->id }}">
                                <i class="fas fa-star"></i> Berikan Ulasan
                            </button>

                            <script>
                            document.querySelector('.review-button[data-id="{{ $reservasi->id }}"]').addEventListener(
                                'click',
                                function() {
                                    Swal.fire({
